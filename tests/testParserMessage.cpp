...
INSTANTIATE_TEST_SUITE_P(
	...
	::testing::Values(
		...
		// 6
		MessageTestParam{
			":Nickname!user@host PRIVMSG #channel :Salut tout le monde, comment ça va ?",
			":Nickname!user@host",
			"PRIVMSG",
			{"#channel", "Salut tout le monde, comment ça va ?"}},
		// 7
		MessageTestParam{
			":Nickname!user@host PRIVMSG #channel :@membre1 @membre2, pouvez-vous jeter un œil à http://exemple.com ? "
            "Aussi, /join #autreCanal pour notre réunion.",
			":Nickname!user@host",
			"PRIVMSG",
			{"#channel",
             "@membre1 @membre2, pouvez-vous jeter un œil à http://exemple.com ? Aussi, /join #autreCanal pour notre "
             "réunion."}},
		// 8
		MessageTestParam{
			":Nickname!user@host MODE #channel +o utilisateur",
			":Nickname!user@host",
			"MODE",
			{"#channel", "+o", "utilisateur"}}));